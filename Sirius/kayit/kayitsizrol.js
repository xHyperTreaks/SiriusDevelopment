...
exports.run = async (client, message, args) => {
  ...
  if (args[0] === "kapat" || args[0] === "sıfırla") {
    if (db.fetch(`kayitsizrol.${message.guild.id}`)) {
      message.channel.send(`Kayıtsız Rol başarıyla kaldırıldı`);
      db.delete(`kayitsizrol.${message.guild.id}`);
      return;
    }
    message.channel.send(`Kayıtsız rol ayarlanmamış.`);
    return;
  }
  
  if (rol) {
	  db.set(`kayitsizrol.${message.guild.id}`, rol.id)
