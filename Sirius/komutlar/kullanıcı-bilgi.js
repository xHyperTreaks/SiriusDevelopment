...
exports.run = async (client, message, args) => {
  ...
  const Durum = user.presence.status;
			...
			const durm = (Durum == "online" ? ("<:cevrimici:781463076343644160> Çevrimiçi") : (Durum == "offline" ? ("<:cevrimdisi:781463076343906304> Çevrimdışı") : (Durum == "idle" ? ("<:bosta:781463076443783178> Boşta") : (Durum == "dnd" ? ("<:rahatsizetme:781463076503420938> Rahatsız Etmeyin") : ("Bilinmiyor/bulunamadı.")))))
    ...
    const embed = new Discord.MessageEmbed()
		...
		.addField("Rolleri:", `${member.roles.cache.filter(r => r.name !== "@everyone").map(r => r).join(' **|** ') ? member.roles.cache.filter(r => r.name !== "@everyone").map(r => r).join(' **|** ') : 'Bu kullanıcının bu sunucuda hiç rolü bulunmuyor.'}`)
